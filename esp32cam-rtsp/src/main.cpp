...
// SoftwareSerial
SoftwareSerial portOne(MYPORT_TX, MYPORT_RX);
...
// MQTT
const char *ssid = "mcu_robot_arm";
const char *password = "mcu_robot_arm_password";
const char *mqtt_server = "47.112.96.50";
...
void on_connected()
{
  ...
  if (camera_init_result == ESP_OK)
  {
    ...
    if (mqttClient.connect("mcu_robot_arm_espcam", ssid, password))
    {
      log_i("mqtt connected");
    }
    else
    {
      log_i("mqtt connected fail");
    }
    mqttClient.subscribe("mcu_robot_arm");
...
void setup()
{
  ...
  // SoftwareSerial init
  portOne.begin(9600);
  // portOne.print("portone print: Init");
  Serial.print("from serial: portone print init");
